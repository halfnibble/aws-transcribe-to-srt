...
#
# Purpose: The program provides a number of utility functions for creating
...
# Purpose: Format and return a string that contains the converted number of
# seconds into SRT format
# Parameters:
#                 seconds - the duration in seconds to convert to HH:MM:SS,mmm
# =============================================================================
# Format and return a string that contains the converted number of seconds into
# SRT format
def getTimeCode(seconds):
    ...
    frac = frac * 1000
    return str(
        "%s,%03d" % (time.strftime("%H:%M:%S", time.gmtime(whole)), frac)
    )


# =============================================================================
# Function: writeTranscriptToSRT
# Purpose: Function to get the phrases from the transcript and write it out to
# an SRT file
# Parameters:
#                 transcript - the JSON output from Amazon Transcribe
#                 sourceLangCode - the language code for the original content
#                 (e.g. English = "EN")
...
# =============================================================================
# Function: writeTranscriptToSRT
# Purpose: Based on the JSON transcript provided by Amazon Transcribe, get the
#          phrases from the translation
#          and write it out to an SRT file
# Parameters:
#                 transcript - the JSON output from Amazon Transcribe
#                 sourceLangCode - the language code for the original content
#                 (e.g. English = "EN")
#                 targetLangCode - the language code for the translated content
#                 (e.g. Spanich = "ES")
#                 srtFileName - the name of the SRT file (e.g. "mySRT.SRT")
# =============================================================================
def writeTranslationToSRT(
    transcript, sourceLangCode, targetLangCode, srtFileName, region
):
    # First get the translation
    print(
        "\n\n==> Translating from " + sourceLangCode + " to " + targetLangCode
    )
    translation = translateTranscript(
        transcript, sourceLangCode, targetLangCode, region
    )
...
# Purpose: Based on the JSON translation provided by Amazon Translate, get the
#          phrases from the translation
#          and write it out to an SRT file.  Note that since we are using a
#          block of translated text rather than
#          a JSON structure with the timing for the start and end of each word
#          as in the output of Transcribe,
...
def getPhrasesFromTranslation(translation, targetLangCode):
    ...
    for word in words:
        ...
        if x == 10:
            ...
            # For Translations, we now need to calculate the end time for the
            # phrase
            psecs = getSecondsFromTranslation(
                getPhraseText(phrase),
                targetLangCode,
                "phraseAudio" + str(c) + ".mp3",
            )
            seconds += psecs
        ...
        # This if statement is to address a defect in the SubtitleClip. If the
        # Subtitles end up being a different duration than the content, MoviePy
        # will sometimes fail with unexpected errors while processing the
        # subclip. This is limiting it to something less than the total
        # duration for our example however, you may need to modify or eliminate
        # this line depending on your content.
        if c == 30:
            break
...
def getPhrasesFromTranscript(transcript):
    ...
    # This function is intended to be called with the JSON structure output
    # from the Transcribe service. However, if you only have the translation of
    # the transcript, then you should call getPhrasesFromTranslation instead
...
# Purpose: Based on the JSON transcript provided by Amazon Transcribe, get the
#          JSON response of translated text
# Parameters:
#                 transcript - the JSON output from Amazon Transcribe
#                 sourceLangCode - the language code for the original content
#                 (e.g. English = "EN")
#                 targetLangCode - the language code for the translated content
#                 (e.g. Spanich = "ES")
#                 region - the AWS region in which to run the Translation
#                 (e.g. "us-east-1")
# =============================================================================
def translateTranscript(transcript, sourceLangCode, targetLangCode, region):
    # Get the translation in the target language.  We want to do this first so
    # that the translation is in the full context of what is said vs. 1 phrase
    ...
    # call Translate  with the text, source language code, and target language
    # code. The result is a JSON structure containing the translated text
...
# Parameters:
#                 phrases - the array of JSON tuples containing the phrases to
...
def writeSRT(phrases, filename):
    ...
    for phrase in phrases:
        ...
        # write out the full phase. Use spacing if it is a word, or punctuation
        # without spacing
...
# Parameters:
#                 phrase - the array of JSON tuples containing the words to
#                          show up as subtitles
# =============================================================================
